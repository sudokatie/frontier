/**
 * Music system for Frontier using Web Audio API.
 * Generates spacey, exploration-themed chiptune background music.
 */
...
class MusicSystem {
  ...
  private playNote(freq: number, startTime: number, duration: number, vol: number = 1): void {
    ...
    // Sine wave for spacey, ethereal feel
    osc.type = 'sine';
  ...
  // Gameplay track - spacey, exploration, cosmic wonder
  private getGameplayNotes(): Note[] {
    ...
    // Cosmic exploration melody (8 bars)
    const melody = [
      // Bar 1-2: Drifting in space
      { note: 'E4', dur: half },
      { note: 'G4', dur: quarter },
      { note: 'B4', dur: quarter },
      { note: 'A4', dur: half },
      { note: 'G4', dur: half },
      // Bar 3-4: Stars passing
      { note: 'D4', dur: quarter },
      { note: 'E4', dur: quarter },
      { note: 'G4', dur: half },
      { note: 'A4', dur: quarter },
  ...
  private getMenuNotes(): Note[] {
    ...
    return [
      ...
      { note: 'D5', dur: beat, volume: 0.5 },
      { note: 'B4', dur: beat * 2, volume: 0.5 },
  ...
  private getGameoverNotes(): Note[] {
    ...
    const beat = 60 / bpm;
    return [
      { note: 'D4', dur: beat, volume: 0.7 },
      { note: 'C4', dur: beat, volume: 0.6 },
      { note: 'A3', dur: beat * 2, volume: 0.4 },
      { note: 'G3', dur: beat * 2, volume: 0.3 },
    ].map(n => ({ frequency: this.noteToFreq(n.note), duration: n.dur, volume: n.volume }));
  }
